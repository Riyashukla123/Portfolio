...
export default function Card({stack, name, index, image, activeCard, setActiveCard}){
  return(
    <div className={`h-full w-full flex flex-col justify-between relative group overflow-hidden`} onMouseEnter={()=>setActiveCard(index)}>
      <Image width={700} height={500} src={image}  alt='projectPic' className={`flex absolute  ${activeCard == index ? 'translate-y-0 md:translate-x-0' : 'translate-y-[-100%] md:translate-x-[100%] md:translate-y-0'} transition duration-900 inset-0 w-full h-full object-cover z-10`}/>
